...
class SensorFusionNode(Node):
    ...
    def imu_listener_callback(self, msg):
        ...
        if 7.6 <= abs(current_heading - self.last_valid_heading) <= 8.4:
			current_heading = self.last_valid_heading
            self.get_logger().warn(f'IMU noise: {current_heading:.2f} (Deviation of 8.0)')
        ...
        if abs(heading_change) < 1.0:
            self.initial_imu_heading += heading_change
